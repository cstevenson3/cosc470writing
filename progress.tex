...
\centering
\includegraphics[width=0.12\textwidth]{mb}
...
Mackay \cite{mackay2019robust} provides the synthetic models he used for testing, with contours sampled on planes cut through the model. I used the "multi-branch" model sampled with 10 and 50 planes, focussing on the middle branch and the lower branch (See Figure \ref{fig:model}). I ran the reconstruction with DTW, and with my point angle method at various weightings for the angle of points vs. the progression of points. Although quantitative accuracy testing is possible (and will be done in future), with the limited time I had I elected to begin with a visual comparison to screen my method for viability (See Figures \ref{fig:plane10} and \ref{fig:plane50}). 
