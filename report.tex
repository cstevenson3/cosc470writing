...
\centering
\includegraphics[width=0.48\textwidth]{diagrams/dtw_slide}
\caption{DTW path finding through a cost matrix to give a point correspondence \label{fig:dtw_slide}}
\end{wrapfigure}

Mackay \cite{mackay2019robust} proposes Dynamic Time Warping (DTW) as a method of point correspondence. DTW is intended to match features on the same structure across different times. In point correspondence, it matches points on contours which are from the same structure but in different slices, so slightly warped. At any time during DTW a pair of points from both contours is matched, then the next points in both contours are considered as matches for the current points. A cost matrix is populated based on point distance, then a path is found aiming to minimise global distance (See Figure \ref{fig:dtw_slide}). DTW has some basic constraints when finding a warp path, but Mackay suggests adding further constraints as a potential improvement. It tends to be that as planes are more separated, the contours undergoing point correspondence are more dissimilar, and DTW performs worse in reconstructing these.
...
\caption{Contour merging for branching cases \label{fig:contour_merging}}
\end{wrapfigure}
