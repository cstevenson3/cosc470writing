...
Images from scans (also referred to as sections or slices) can be segmented into pixel regions to define structure boundaries. This can be processed further by finding contours to represent these boundaries.
...
Pan et al. \cite{pan2017comparison} refers to the importance of 3D reconstructions and rendering in robot-assisted surgery. 
...
Segmentation is the process of picking out the pixels belonging to individual objects in an image. From this the projected geometry of the object onto the image can be inferred, which is then used in reconstruction or rendering.
